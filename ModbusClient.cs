...
public class ModbusClient
{
    ...
    public void Connect()
    {
        ...
        if (!UdpFlag)
        {
            ...
            if (!success) throw new ConnectionException("连接超时");
    ...
    public bool[] ReadDiscreteInputs(int startingAddress, int quantity)
    {
        ...
        if (_serialPort is { IsOpen: false }) throw new SerialPortNotOpenedException("串行端口未打开");

        if ((_tcpClient == null) & !UdpFlag & (_serialPort == null)) throw new ConnectionException("连接错误");
        ...
        int[] values)
    {
        ...
        if ((startingAddressRead > 65535) | (quantityRead > 125) | (startingAddressWrite > 65535) |
            (values.Length > 121)) throw new ArgumentException("起始地址必须为 0 - 65535；查询的数量必须为 1 - 125；写入的数量必须为 1 - 121");
    ...
    public static int[] ConvertStringToRegisters(string stringToConvert)
    {
        ...
        for (var i = 0; i < toRegisters.Length; i++)
        {
            ...
            if (i * 2 + 1 < array.Length) toRegisters[i] |= array[i * 2 + 1] << 8;
        }
    ...
    /// </summary>
    /// <returns></returns>
    public static int[] SwapRegisters(params int[] registers)
    {
        var swappedRegisters = new int[registers.Length];
        for (var i = 0; i < registers.Length; i++)
        {
            var word = (short)registers[i];
            var bytes = BitConverter.GetBytes(word);
            swappedRegisters[i] = BitConverter.ToInt16([bytes[1], bytes[0]], 0);    
        }
