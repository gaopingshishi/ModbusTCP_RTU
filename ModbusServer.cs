...
internal class TcpHandler
{
    ...
    public TcpHandler(int port, long timeout = 0, List<string>? ipWhitelisting = null)
    {
        _timeout = timeout;
    ...
    /// <param name="timeout">超时时间，如果客户端在此时间内没有发送数据，则断开连接。 单位：ms ，设置为 0 代表不启用超时。</param>
    /// <param name="ipWhitelisting">IP白名单，为空代表不启用</param>
    public TcpHandler(IPAddress localIpAddress, int port, long timeout = 0, List<string>? ipWhitelisting = null)
    {
        _timeout = timeout;
        IpWhitelisting = ipWhitelisting ?? [];
    ...
    private int GetAndCleanNumberOfConnectedClients(Client client)
    {
        lock (this)
        {
            ...
            foreach (var _ in _tcpClientLastRequestList.Where(client.Equals)) objetExists = true;
    ...
    public void Disconnect()
    {
        try
        {
            foreach (var clientLoop in _tcpClientLastRequestList) clientLoop.NetworkStream.Close(00);
        }
        catch (Exception)
        {
            // ignored
...
{
    private readonly object _lockCoils = new();
    private readonly object _lockHoldingRegisters = new();
    ...
    ///     IP地址白名单，如果不为空，则只有白名单中的IP地址才能连接到服务器。
    /// </summary>
    public List<string> IpWhitelisting { get; } = [];
...
#pragma warning restore SYSLIB0006
    ...
    private void ListenerThread()
    {
        if (!UdpFlag & !SerialFlag)
        {
            ...
            _tcpHandler = new TcpHandler(LocalIpAddress, Port, ipWhitelisting: IpWhitelisting);
...
#pragma warning restore CS8602 // 解引用可能出现空引用。
    ...
    {
        for (var i = 0; i < 98; i++) ModbusLogData[99 - i] = ModbusLogData[99 - i - 2];
